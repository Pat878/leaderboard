...
class LeaderBoard extends React.Component {
  ...
  render() {
    const namesToRender = this.state.name.map(val => {
      return <tr key={val.username}> {val.username}</tr>;
    });

    const imgsToRender = this.state.name.map(val => {
      return (
        <tr key={val.username}>
          <img src={val.img} height="18" alt={val.img} />
        </tr>
      );
    });

    const allTimeRender = this.state.name.map(val => {
      return <tr key={val.username}> {val.alltime}</tr>;
    });

    const recentRender = this.state.name.map(val => {
      return <tr key={val.username}> {val.recent}</tr>;
    });
    ...
    return (
      <div className="container">
        <center>
          ...
          <Button
            bsClass={this.state.active ? "btn active" : "btn"}
            onClick={this.allTimeClick}
          >
            Alltime
          </Button>
          <Button
            bsClass={this.state.active ? "btn" : "btn active"}
          ...
          <thead>
            <tr>
              ...
              <th>
                ...
                <div
                  className={
                    this.state.active ? "fa fa-arrow-up" : "fa fa-arrow-down"
                  }
                />
              </th>
              <th>
                ...
                <div
                  className={
                    this.state.active ? "fa fa-arrow-down" : "fa fa-arrow-up"
                  }
                />
              </th>
            </tr>
